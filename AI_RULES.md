...
7. **­ƒÜ½ Envio Sem Valida├º├úo Local (Fail-Fast):** ├ë OBRIGAT├ôRIO que qualquer altera├º├úo em `apps/mobile/` passe por `pnpm --filter @painel-prime/mobile run check:local` antes de commit ou push para qualquer remote (`origin` ou `mobile`). O script executa `tsc --noEmit` (tipagem) seguido de `npx expo prebuild --dry-run` (valida├º├úo de `app.json`, plugins e configura├º├úo nativa). Se qualquer etapa falhar, o fluxo DEVE ser interrompido e corrigido localmente. NUNCA delegue a descoberta de erros de sintaxe, imports fantasmas ou falhas de `expo config` para a esteira do GitHub Actions.
8. **­ƒöÑ Smoke Test com Dispositivo (Pr├®-Commit Complexo):** Antes de commits que envolvam altera├º├Áes de UI, roteamento, depend├¬ncias nativas ou `app.json`, execute `pnpm --filter @painel-prime/mobile run check:smoke`. Este script faz a checagem de tipos (`tsc --noEmit`) e em seguida sobe o Metro Bundler enviando o bundle diretamente para o dispositivo Android conectado via ADB (`npx expo start --android`). O bundle JavaScript DEVE compilar at├® 100% sem erros no console. S├│ fa├ºa push ap├│s o smoke test passar limpo.
